...
function onTriggered() {
    ...
    function animationCallback() {
        if (script.AnimationMixer.getLayer(script.LayerName)) {
            script.AnimationMixer.getLayer(script.LayerName).weight = 0.0;
        }
    }
    ...
    if (script.AnimationMixer && script.LayerName) {
        if (script.AnimationMixer.getLayer(script.LayerName)) {
            ...
            script.AnimationMixer.getLayer(script.LayerName).weight = 1.0;
        }
    }
    ...
    for (var j = 0; j < script.playAudios.length; j++) {
        if (script.playAudios[j]) {
            var audioComponent = getAudioComponentForTrack(script.playAudios[j]);
